function abs_dispersion_uvw_seas2(grdnum,f,level,seas)
...
%for r = 12
    ...
    tic;
    parfor p = 1:pp
        t_temp = find(isnan(sq(lon(:,1,p))) == 1,1,'first');
        if ~isempty(t_temp); tt = t_temp - 1; else tt = t_size+1; end
        disp(['Particle ',num2str(p),' life = ',num2str(tt)])        
        upts = sq(u_primes(:,1,p)); vpts = sq(v_primes(:,1,p)); wpts = sq(w_primes(:,1,p)); 
%         if tt > 2;
%             for to = 2:tt-1
%                 %disp(to)
%                 for tf = to+1:tt
...
%             end
%         end
        [dispu(:,p),dispv(:,p),dispw(:,p),disp2d(:,p),nlag(:,p)] = ...
            calc_disp(upts,vpts,wpts,tt,mo,s,lon(:,1,p),lat(:,1,p));
    ...
    filenames = ['dispuvw_seas_',str,'_',flstr(1:2),'_',timestr,'_',num2str(grdnum),num2str(level),seas_str];
...
disp2d = zeros(ttt-1,1); ndata = zeros(1,ttt-1);
